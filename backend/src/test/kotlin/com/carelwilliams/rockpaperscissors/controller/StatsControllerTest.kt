...
internal class StatsControllerTest @Autowired constructor(val mockMvc: MockMvc, val objectMapper: ObjectMapper) {
    ...
    fun postStatsBeforeTest(userId: String) {
        ...
        val STATS = GameStats(
            userId = userId,
            listOf(RockPaperScissor.SCISSOR),
            winLoseRatio = 3.15,
            totalWins = 20,
            historicPicks = listOf(RockPaperScissor.ROCK, RockPaperScissor.PAPER),
            totalLosses = 23,
    ...
    inner class GetAllStats {
        ...
        fun `Should return all stats`() {
            ...
            mockMvc.get(baseUrl).andDo { print() }.andExpect {
                ...
                jsonPath("$[0].winLoseRatio") { value("3.15") }
            }
        }
    }
    ...
    inner class PostNewStat {
        ...
        fun `should add the new stat`() {
            ...
            fun `should return BAD REQUEST if a userId already exists`() {
                ...
                val secondPost = mockMvc.post(baseUrl) {
                    contentType = MediaType.APPLICATION_JSON
                    content = objectMapper.writeValueAsString(OTHER_STAT)
                }
                secondPost
        ...
        inner class PUTStats {
            ...
            fun `Should update existing resource`() {
                ...
                val updateStat = UpdateGameStats(
                    picks = listOf(RockPaperScissor.SCISSOR),
                    didUserWin = true,
                    playTime = 600,
                )

                val performPut = mockMvc.put("$baseUrl/${userId}") {
                    contentType = MediaType.APPLICATION_JSON
